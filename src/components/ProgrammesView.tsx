...
import { Member, Branch, AuditLog, PermissionMatrix, Delegation } from '../types';
...
export default function ProgrammesView({
  ...
  // Get members of active branch, cloisonnés par département (même règle que CursusView/MembersView)
  const activeBranchMembers = members.filter(m =>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-5 rounded-[2rem] border border-bc-border shadow-sm space-y-4">
          <h4 className="text-xs uppercase font-bold tracking-wider text-bc-text-secondary flex items-center gap-2">
            <Clock size={13} /> Candidats en cours de parcours de Baptême ({unbaptisedMembers.length})
          </h4>
              ...
              unbaptisedMembers.map((m) => (
                <div key={m.id} className="border border-bc-border rounded-2xl p-4 space-y-3 bg-bc-canvas/20">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center space-x-2.5">
                      <Avatar
                        ...
                        size="sm"
                        className="w-8 h-8 bg-bc-canvas border border-bc-border text-[10px]"
                      />
                      <div>
                  ...
                  <div className="grid grid-cols-4 gap-1 text-[8px] text-center font-bold">
                    <div className="p-1 rounded-lg bg-bc-green text-white">1. Inscription</div>
                    <div className="p-1 rounded-lg bg-bc-green text-white">2. Cours</div>
                    <div className="p-1 rounded-lg bg-bc-green text-white">3. Entretien</div>
                    <div className="p-1 rounded-lg bg-bc-canvas text-bc-text-secondary border border-bc-border">4. Immersion</div>
                  </div>
                  ...
                  {canValidateBaptism && (
                    <div className="flex justify-end pt-2 border-t border-bc-border/50">
                      <button
                        ...
                        className="px-3 py-1 bg-bc-gold text-bc-text font-ui font-bold text-[10px] rounded-full hover:scale-105 transition-all flex items-center gap-1 cursor-pointer"
                      >
        ...
        <div className="bg-white p-5 rounded-[2rem] border border-bc-border shadow-sm space-y-4">
          <h4 className="text-xs uppercase font-bold tracking-wider text-bc-text flex items-center gap-2">
            <Award size={13} /> Fidèles baptisés physiques ({baptisedMembers.length})
          </h4>
          ...
          <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-1">
            {baptisedMembers.length === 0 ? (
              ...
              baptisedMembers.map((m) => (
                <div key={m.id} className="border border-bc-border rounded-2xl p-3 flex justify-between items-center bg-white hover:bg-bc-canvas/10">
